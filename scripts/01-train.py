...
if args_cli.config:
    ...
    cli_keys = ["task", "seed", "max_iterations", "video", "video_length", "video_interval",
                "resume", "load_run", "checkpoint", "run_name", "experiment_name", "logger",
                "log_project_name"]
    for key in cli_keys:
...
def enable_base_height_metrics(runner: OnPolicyRunner) -> None:
    """Log physical terrain-relative base height statistics for each rollout."""
    height_sum = 0.0
    abs_error_sum = 0.0
    sample_count = 0
    original_process_env_step = runner.logger.process_env_step
    original_log = runner.logger.log

    def process_env_step_and_height(*args, **kwargs):
        nonlocal height_sum, abs_error_sum, sample_count

        height = getattr(runner.env.unwrapped, "_last_base_height_above_terrain", None)
        target = getattr(runner.env.unwrapped, "_base_height_target", None)
        if height is not None and target is not None:
            valid_height = height[torch.isfinite(height)]
            if valid_height.numel() > 0:
                height_sum += valid_height.sum().item()
                abs_error_sum += torch.abs(valid_height - target).sum().item()
                sample_count += valid_height.numel()

        original_process_env_step(*args, **kwargs)

    def log_with_height(**kwargs):
        nonlocal height_sum, abs_error_sum, sample_count

        original_log(**kwargs)
        if sample_count > 0 and runner.logger.writer is not None:
            iteration = kwargs["it"]
            runner.logger.writer.add_scalar(
                "Metrics/mean_base_height_above_terrain_m", height_sum / sample_count, iteration
            )
            runner.logger.writer.add_scalar(
                "Metrics/mean_abs_base_height_error_m", abs_error_sum / sample_count, iteration
            )
    ...
    runner.logger.process_env_step = process_env_step_and_height
    runner.logger.log = log_with_height
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if isinstance(runner, OnPolicyRunner):
        ...
        enable_base_height_metrics(runner)
